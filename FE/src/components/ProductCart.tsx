...
const ProductCart = ({ product }: Props) => {
    ...
    return (
        <div key={product._id} className="grid grid-cols-[100%] lg:auto-rows-[240px_auto] mb:auto-rows-[160px_auto] lg:gap-y-6 mb:gap-y-[27px] overflow-hidden h-full rounded-xl">
            {/* img */}
            {contextHolder}
            ...
            <div className="w-full flex flex-col justify-between gap-y-4 items-center">
                ...
                <div className="flex flex-col gap-y-2 items-center lg:translate-y-0 mb:-translate-y-[2.5px]">
                    <div className="flex mb:text-sm lg:text-base lg:mb-3 mb:mb-2.5">
                        <span className=" text-[#EB2606]">$ {product.price}</span><span></span>
                    </div>
                    <button onClick={() => addToCart(product)} className="bg-[#17AF26] lg:w-[128px] mb:w-[118px] lg:mt-[11px] mb:mt-[14.5px] h-[40px] grid place-items-center rounded-[100px] lg:text-sm mb:text-xs text-white">
                        Add to Cart
                    </button>
                </div>
            </div>
